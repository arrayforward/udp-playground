...
//-------------------------------------------------------------------------------------------------------------------------------
    ...
    // Arguments and return values are passed in and out through the stack pointer wasm_sp.
    // Placeholder return value slots are first and arguments after. So, the first argument is at wasm_sp [numReturns]
    // Return values should be written into wasm_sp [0] to wasm_sp [num_returns - 1]
    typedef const void * (* M3RawCall) (IM3Runtime runtime, IM3ImportContext _ctx, uint64_t * wasm_sp, void * _mem);
...
# define m3ApiReturnType(TYPE)      TYPE* raw_return = ((TYPE*) (wasm_sp++));
# define m3ApiGetArg(TYPE, NAME)    TYPE NAME = * ((TYPE *) (wasm_sp++));
# define m3ApiGetArgMem(TYPE, NAME) TYPE NAME = (TYPE)m3ApiOffsetToPtr(* ((uint32_t *) (wasm_sp++)));
...
# define m3ApiRawFunction(NAME)     const void * NAME (IM3Runtime runtime, IM3ImportContext _ctx, uint64_t * wasm_sp, void * _mem)
